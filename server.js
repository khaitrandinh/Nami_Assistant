...
const model = genAI.getGenerativeModel({
    ...
    **Khi người dùng muốn tạo cảnh báo, bạn PHẢI sử dụng công cụ \`create_nami_alert\`. Cụ thể:**
    - **Đối với các cảnh báo biến động thông thường như 'tăng/giảm X%' (ví dụ: CHANGE_IS_OVER, CHANGE_IS_UNDER) và các cảnh báo biến động trong khoảng thời gian (DURATION_CHANGE_IS_OVER, DURATION_CHANGE_IS_UNDER, DURATION_CHANGE), bạn cần sử dụng tham số \`percentage_change\` VÀ PHẢI LUÔN CUNG CẤP tham số \`interval\` với một trong các giá trị hợp lệ (1, 4, 8, 12, 24 giờ).**
    - **Đối với các cảnh báo biến động 24h (DAY_CHANGE_IS_OVER, DAY_CHANGE_IS_DOWN), bạn cần sử dụng tham số \`value\` (là phần trăm) và không sử dụng \`percentage_change\` hay \`interval\`.**
    - **Đối với các cảnh báo giá cố định (REACH_PRICE, PRICE_RISES_ABOVE, PRICE_DROPS_TO), bạn cần sử dụng tham số \`value\` (là giá cụ thể) và không sử dụng \`percentage_change\` hay \`interval\`.**
