...
const model = genAI.getGenerativeModel({
    ...
    **Do đó, bạn BẮT BUỘC phải sử dụng các công cụ của mình để truy xuất dữ liệu từ API Nami trước khi trả lời bất kỳ câu hỏi nào về một token cụ thể, giá cả, thông tin liên quan đến Nami, HOẶC CÁC CÂU HỎI VỀ TIN TỨC/BLOG/KHUYẾN MÃI/XU HƯỚNG TỪ NAMI, HOẶC THÔNG TIN PORTFOLIO CỦA NGƯỜI DÙNG.**
    ...
    **Bạn PHẢI VÀ CHỈ ĐƯỢC trả lời bằng ngôn ngữ mà người dùng đã sử dụng để đặt câu hỏi và không quan tâm đên lịch sử chat là ngôn ngữ gì.**
    ...
    **Nếu người dùng hỏi về hiệu suất portfolio, hãy cung cấp tổng quan portfolio, tỷ lệ phân bổ tài sản, và hiệu suất 24h của các tài sản chính. Lưu ý: Giá trị portfolio sẽ được tính bằng đồng tiền cơ sở được chỉ định (mặc định là VNST). Thông tin xu hướng giá theo tuần/tháng hiện không khả dụng.**
        ...
        while (hasFunctionCall) {
            ...
            if (!func) {
                ...
                if (call.name === 'get_nami_token_info') {
                    ...
                    // Truyền name_currency (chuỗi) và userLang
                    apiResult = await func(userLang, call.args.name_currency || 'VNST'); // ĐẢO THỨ TỰ THAM SỐ ĐỂ KHỚP VỚI HÀM MỚI
                }
                else {
